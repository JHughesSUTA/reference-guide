...
LoanerUtils.prototype = {
  ...
  initialize: function () {
    ...
    this.returnRemindTime =
      gs.getProperty("x_cdltd_loaner_r_0.return.remindtime", 24) *
      60 *
      60 *
      1000;
    this.overdueRemindTime =
      gs.getProperty("x_cdltd_loaner_r_0.overdue.remindtime", 24) *
      60 *
      60 *
      1000;
  ...
  sendPickupReminder: function (id) {
    ...
    if (!gr.get(id)) {
      gs.debug(
        ">>>DEBUG: LoanerUtils: sendPickupReminder(): FAILED TO RETRIEVE RECORD"
      );
      return;
    }
  ...
  sendReturnReminder: function (id) {
    ...
    if (!gr.get(id)) {
      gs.debug(
        ">>>DEBUG: LoanerUtils: sendReturnReminder(): FAILED TO RETRIEVE RECORD"
      );
      return;
    }
  ...
  timeToRemindOverdue: function (id) {
    ...
    if (!gr.get(id)) {
      gs.debug(
        ">>>DEBUG: LoanerUtils: timeToRemindOverdue(): FAILED TO RETRIEVE RECORD"
      );
      return false;
    }
    ...
    var baseDate =
      gr.last_overdue_reminder == "" ? gr.end_date : gr.last_overdue_reminder;
  ...
  sendOverdueReminder: function (id) {
    ...
    if (!gr.get(id)) {
      gs.error(
        ">>>DEBUG: LoanerUtils: setLastOverdueReminder(): FAILED TO RETRIEVE RECORD"
      );
      return;
    }
